    ...
    lfunction generate_labels (tree_name) {
        ...
        for (node = 1; node < Abs (tip_avl) - 1; node += 1) {
            ...
            if ((tip_avl[node])["Length"] <= 1e-8) {
                assert (zero_branch_lock / parent_name == FALSE || labels [parent_name] == my_label ,
                        "Can't have two zero branch length (same parent) with labels that don't match: `(tip_avl[node])["Name"]` (`my_label`) and `zero_branch_lock [parent_name]` (`labels [parent_name]`)");
            ...
            if (labels / parent_name) { // need to merge
                if ( zero_branch_lock / parent_name  == FALSE) {
                    ...
                    assert (shared > 0, "Reference error: incompatible labels in reference tree `tree_name`; they all MUST share a prefix of at least one letter: `current_parent_label` and `my_label`");
        ...
        if (cache / index) {
            ...
            if (io.FileExists (data_path) == FALSE) {
                ...
                if (io.FileExists (fit_path) == FALSE) {
                    ...
                    fprintf (fit_path, CLEAR_FILE, result);
                } else {
                ...
                fprintf (data_path, CLEAR_FILE, branch_level_conditionals);
                DeleteObject (^fitted);
            } else {
...
namespace IgSCUEAL {
   ...
   lfunction handle_sequence_alignment_1 (seq, ref, options) {
    ...
    lfunction  load_reference_alignment (file_path, filter, label) {
       ...
       data  = alignments.ReadCodonDataSetFromPathGivenCode (namespace, io.MakeAbsoluteFilePath (file_path),
